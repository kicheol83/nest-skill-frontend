...
const FaqArticles: NextPage = (props: any) => {
  ...
  const {
    ...
    refetch: getAllNoticeByAdminRefetch,
    ...
    onCompleted: (data: T) => {
      ...
      const filteredNotices = allNotices.filter(
        (n: any) => n.noticeCategory === NoticeCategory.NOTICE
      );
  ...
  /** LIFECYCLES **/
  useEffect(() => {
    getAllNoticeByAdminRefetch({ input: faqInquiry });
  }, [faqInquiry]);

  /** HANDLERS **/
  const changePageHandler = async (event: unknown, newPage: number) => {
    faqInquiry.page = newPage + 1;
    await getAllNoticeByAdminRefetch({ input: faqInquiry });
  ...
  const removeFaqHandler = async (id: string) => {
    try {
      ...
      await sweetTopSuccessAlert("Delete succesfully!");
      await getAllNoticeByAdminRefetch({ input: faqInquiry });
  ...
  const updateFaqHandler = async (updateData: NoticeUpdate) => {
    try {
      ...
      await sweetTopSuccessAlert("Update succesfully!");
      await getAllNoticeByAdminRefetch({ input: faqInquiry });
      menuIconCloseHandler();
    } catch (err: any) {
      menuIconCloseHandler();
                      ...
                      {searchText && (
                        <CancelRoundedIcon
                          ...
                          onClick={async () => {
                            ...
                            await getAllNoticeByAdminRefetch({
                              input: faqInquiry,
                            });
                          }}
                        />
